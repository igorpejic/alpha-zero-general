...
class BinPackNNet():
    def __init__(self, game, args):
        ...
        with self.graph.as_default():
            ...
            x_image = tf.reshape(self.input_boards, [-1, self.board_x, self.board_y, self.channels])                    # batch_size  x board_x x board_y x 1
            ...
            h_conv4 = Relu(BatchNormalization(self.conv2d(h_conv1, args.num_channels, 'valid'), axis=3, training=self.isTraining))     # batch_size  x (board_x-4) x (board_y-4) x num_channels
            ...
            self.v = tf.nn.sigmoid(Dense(s_fc2, 1))                                                               # batch_size x 1
